...
class TradeIndex():
    ...
    def _set_dates(self, dates : List[str], format :str='YYYY-MM-DD'):
        if format not in self.AVAILABLE_DATE_FORMATS:
            raise KeyError('Such date format is not supported!\n' + 
        ...
        while check_idx < len(format):
            if format[check_idx] == 'Y':
                try:
                    ...
                    print(f'Year of date {date} is specified incorrectly ' + 
                ...
                try:
                    ...
                    print(f'Month of date {date} is specified incorrectly ' + 
                          f'from the given format {format}!')
                ...
                try:
                    ...
                    print(f'Day of date {date} is specified incorrectly ' + 
                          f'from the given format {format}!')
            else:
            ...
            if check_idx < len(format):
                self._check_date_separator(date=date,
                                        date_sep=date[check_idx],
                                        format_sep=format[check_idx])
                check_idx += 1
    ...
    def _check_date_separator(self, date : str, date_sep : str, format_sep : str):
        if date_sep != format_sep:
            raise ValueError(f'Separator of date {date} is specified incorrectly' + 
